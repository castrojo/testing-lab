...
- A GitLab MR source tracker that URL-encodes the project path with a trailing newline — the API returns 404. Use `printf '%s' "$project" | jq -sRr @uri`, validate the returned SHA is a 40-character hex string, and switch from the MR head SHA to the target branch HEAD only when `.state == "merged"`.
