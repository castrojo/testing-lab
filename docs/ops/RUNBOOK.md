...
| Container QA fails in `wait_for_shell.py` with `Shell.Eval not ready: ... Could not connect: No such file or directory` after 30 attempts | **OPEN.** `qecore-headless` stops and restarts GDM before invoking the runner script, so the session the runner just provisioned is torn down. The readiness probe sees the stale bus for ~14 attempts (`ServiceUnknown`), then the socket disappears (`Could not connect`) and the replacement autologin session does not register inside the ~60s that `wait_for_shell.py` allows | The env vars were corrected to the logind-managed runtime dir (`/run/user/1000`, `unix:path=/run/user/1000/bus`) — necessary but **not sufficient**. Reproduced solo (`probe-solo-pr718-q2r6g`) and with a settle-wait for `gnome-shell` + `/run/user/1000/bus` (`probe-settle-p52b7`): both still fail, because the socket dies *after* the settle check passes. The retry budget lives in `tests/shared/wait_for_shell.py` in `projectbluefin/testsuite`; fixing it requires that repo to tolerate the bus socket disappearing and re-appearing across a GDM restart. Do not chase this in `lab` |
