...
| Container QA fails in `wait_for_shell.py` with `Shell.Eval not ready: ... Could not connect: No such file or directory` after 30 attempts | `qecore-headless` stops and restarts GDM, so the session created during provisioning is torn down and a fresh one is started under `/run/user/1000`. The runner was still exporting `XDG_RUNTIME_DIR`/`DBUS_SESSION_BUS_ADDRESS`/`AT_SPI_BUS_ADDRESS` pointing at `/home/bluefin-test/run`, which no longer holds a bus socket | Point all three env vars at the logind-managed runtime dir (`/run/user/1000`, `unix:path=/run/user/1000/bus`) in `run-container-tests` |
