...
## history/cache-heat.ndjson

Rolling cache-heat evidence written by `scripts/collect_bst_cache.py`. One line
per cache backend and BuildBarn storage type per collection. The file is
retained for 180 days and is safe to consume as a dashboard trend source.

| Field | Type | Meaning |
| --- | --- | --- |
| `schema_version` | `"1.0"` | Cache-heat contract version |
| `recorded_at` | ISO8601 | Collection time |
| `cache_backend` | string | `buildbarn` or `bazel-remote` |
| `pod` | string or null | BuildBarn storage pod, when applicable |
| `storage_type` | string | `cas` or `ac` |
| `hit_count` / `miss_count` | integer or null | BuildBarn `Get` results with gRPC `OK` / `NotFound` status |
| `effectiveness` | number or null | `hit_count / (hit_count + miss_count)` when both exist |
...
BuildBarn does not expose dedicated cache-hit counters. The collector derives
...
- `schema_version`: `v1`
- `_meta`: page metadata and `status` (`ready` or `unavailable`)
- `window_days`: trailing retention window used for aggregation (180)
- `lane`: `dakota-testing`
- `summary_metrics[]`: counts with the shared provenance fields
- `rows[]`: one UTC calendar-day row per Dakota `record_type`

Each trend row contains:

| Field | Meaning |
| --- | --- |
| `id` | Stable key (`build-2026-07-28` or `publish-2026-07-28`) |
| `record_type` / `date` | Raw operation type and UTC start date bucket |
| `throughput` | Number of validated runs in the bucket |
| `passed` / `failed` | Terminal status counts |
| `duration_seconds` | `{p50, p95, avg}` derived from raw `duration_min` |
| `state` / `state_reason` | Explicit availability state; no synthetic values |
| `source_url` / `collected_at` / `derivation` | Raw-history provenance |

When no validated Dakota records exist in the retention window, `rows` is
empty, the artifact status is `unavailable`, and summary metric values are
`null` with an explicit `state_reason`.
